...
/**
 * 001_initial.js — complete schema as of initial versioned migration.
 *
 * Contains every CREATE TABLE, ALTER TABLE, CREATE INDEX, and one-time
 * data backfill that was previously in the monolithic migrations.js.
 *
 * All DDL statements are idempotent (IF NOT EXISTS / try-catch) so this
 * migration is safe to apply against an existing production database —
 * everything that already exists will be silently skipped.
 */
...
function up(db) {
  ...
  // ── ALTER TABLE additions (all wrapped in try/catch for idempotency) ───────
  const alters = [
  ...
  for (const sql of alters) {
    try { db.exec(sql); } catch { /* column/table already exists — skip */ }
  }

  // ── One-time data backfills ────────────────────────────────────────────────
  // These run inside the same migration transaction.
  // All are wrapped in try/catch so a failed backfill never blocks startup.

  // Assign role='admin' to the creator of each existing group chat
  try {
    db.exec(`
    ...
    `);
    console.log('[migrate 001] role backfill complete');
  } catch (e) {
    console.warn('[migrate 001] role backfill skipped:', e.message);
  }

  // Compute unread_count for each chat member based on messages after last_read_at
  try {
  ...
  // Decrypt and backfill search_text for messages that predate the column
  try {
    const { decrypt } = require('../../crypto/aes');
    const rows = db.prepare(
      `SELECT id, ciphertext, iv, auth_tag FROM messages
       WHERE search_text IS NULL AND is_system = 0 AND deleted_at IS NULL`
    ).all();
    ...
    console.warn('[migrate 001] search_text backfill skipped:', e.message);
  }

  // Populate FTS5 index if empty (first boot or after rebuild)
  try {
    ...
    if (ftsCount === 0) {
      ...
      if (n > 0) console.log(`[migrate 001] FTS5 index populated: ${n} messages`);
    }
  } catch (e) {
    console.warn('[migrate 001] FTS5 backfill skipped:', e.message);
  }
}
