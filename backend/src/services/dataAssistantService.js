...
const SEMANTIC_SYSTEM =
  ...
  '1) Отвечай НА ТОМ ЖЕ ЯЗЫКЕ, на котором задан вопрос пользователя (например, ' +
  'если вопрос на английском — отвечай на английском, даже если сами сообщения на ' +
  'русском), разверни ответ (2–5 предложений) — не ограничивайся одним словом или ' +
  'обрывочной фразой, дай связный ответ с нужным контекстом (кто, когда, по какому ' +
  'поводу — если это есть в сообщениях). При этом используй ТОЛЬКО факты из этих ' +
  'сообщений и не добавляй домыслов или предположений сверх того, что там написано ' +
  '— при нехватке информации честно скажи, чего не хватает, вместо того чтобы ' +
  'додумывать. Если в них есть дата/время/место/факт — приведи их.\n' +
