...
export function AddToCollectionModal({ chatId, messageId, onClose, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    listCollections(chatId)
      ...
      .catch(() => { if (alive) setError('Не удалось загрузить коллекции'); })
  ...
  async function addTo(col: Collection) {
    ...
    try {
      ...
      setError('Не удалось добавить в коллекцию');
      setBusyId(null);
    }
  }
    ...
    <div className="atcOverlay" onClick={onClose}>
      <div className="atcModal" onClick={e => e.stopPropagation()}>
        <div className="atcHead">
          <span className="atcTitle">Добавить в коллекцию</span>
        ...
        <div className="atcList">
          {loading ? (
            ...
            <div className="colEmpty">Коллекций пока нет — создайте первую.</div>
          ) : (
            cols.map(col => (
        ...
        {creating ? (
          <form className="colCreateRow" onSubmit={e => { e.preventDefault(); handleCreate(); }}>
            <input autoFocus className="colCreateInput" placeholder="Название новой коллекции"
              value={newName} maxLength={80} onChange={e => setNewName(e.target.value)} />
            <button type="submit" className="colAddBtn" disabled={busyId !== null}>Создать и добавить</button>
          </form>
        ) : (
