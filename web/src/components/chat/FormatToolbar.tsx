...
export function FormatToolbar({ textareaRef, value, onChange }: Props) {
  ...
  // After any toolbar button press, freeze readSel for 300ms so that
  ...
  // Uses onPointerDown (fires at touchstart time on mobile, before the browser
  // can process focus changes) instead of onMouseDown to reliably prevent
  // the textarea from losing focus / selection on both desktop and mobile.
  const noBlur = (e: React.PointerEvent) => {
    e.preventDefault();
    freezeUntil.current = Date.now() + 300;
  };

  // Container-level guard: prevent ANY non-input pointer interaction inside
  // the toolbar from stealing focus from the textarea.
  const containerPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    if ((e.target as Element).closest('input')) return; // link URL input needs real focus
    e.preventDefault();
  };
      ...
      {menu === 'format' && (
        <>
          <div className="fmtRow">
            ...
            <div className="fmtSep" />
            {/* Bold */}
            <button className={`fmtBtn fmtBtnFmt${isBoldActive ? ' fmtBtnActive' : ''}`} onPointerDown={e => { noBlur(e); toggleWrap('**', isBoldActive); }} title="Жирный">
              <strong>Ж</strong>
            </button>
            <div className="fmtSep" />
            {/* Italic */}
            <button className={`fmtBtn fmtBtnFmt${isItalicActive ? ' fmtBtnActive' : ''}`} onPointerDown={e => { noBlur(e); toggleWrap('_', isItalicActive); }} title="Курсив">
              <em>К</em>
            </button>
            <div className="fmtSep" />
            {/* Spoiler */}
            <button className={`fmtBtn fmtBtnFmt fmtBtnSpoiler${isSpoilerActive ? ' fmtBtnActive' : ''}`} onPointerDown={e => { noBlur(e); toggleWrap('||', isSpoilerActive); }} title="Скрытый текст">
